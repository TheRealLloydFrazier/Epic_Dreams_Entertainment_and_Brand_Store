...
export async function POST() {
  try {
    ...
    await prisma.$transaction(async (tx: Prisma.TransactionClient) => {
      await tx.product.create({
        data: {
          ...
          variants: {
            create: [
              { name: 'Black / S', sku: 'TEE-BLK-S', priceCents: 3200, inventory: 25, attributes: { size: 'S', color: 'Black' } },
              { name: 'Black / M', sku: 'TEE-BLK-M', priceCents: 3200, inventory: 35, attributes: { size: 'M', color: 'Black' } },
              { name: 'Black / L', sku: 'TEE-BLK-L', priceCents: 3200, inventory: 32, attributes: { size: 'L', color: 'Black' } },
              { name: 'Black / XL Signed', sku: 'TEE-BLK-XL-SIGNED', priceCents: 4800, inventory: 10, signed: true, attributes: { size: 'XL', color: 'Black', signed: true } }
            ]
          }
        }
      });
      ...
      await tx.product.create({
        data: {
          ...
          variants: {
            create: [
              { name: 'Black / M', sku: 'HOOD-BLK-M', priceCents: 6800, inventory: 20, attributes: { size: 'M', color: 'Black' } },
              { name: 'Black / L', sku: 'HOOD-BLK-L', priceCents: 6800, inventory: 18, attributes: { size: 'L', color: 'Black' } },
              { name: 'Black / XL Signed', sku: 'HOOD-BLK-XL-SIGNED', priceCents: 8800, inventory: 5, signed: true, attributes: { size: 'XL', color: 'Black', signed: true } }
            ]
          }
        }
      });
      ...
      await tx.product.create({
        data: {
          ...
          variants: {
            create: [
              { name: 'Standard Edition', sku: 'POST-EMPTY-STD', priceCents: 2600, inventory: 40, attributes: { size: '18x24', finish: 'Matte' } },
              { name: 'Signed Edition', sku: 'POST-EMPTY-SIGNED', priceCents: 4600, inventory: 8, signed: true, attributes: { size: '18x24', finish: 'Matte', signed: true } }
            ]
          }
        }
      });
      ...
      await tx.product.create({
        data: {
          ...
          variants: {
            create: [
              { name: 'One Size', sku: 'HAT-NEON-OS', priceCents: 3600, inventory: 50, attributes: { size: 'OS', color: 'Black' } }
            ]
          }
        }
      });
      ...
      await tx.product.create({
        data: {
          ...
          variants: {
            create: [
              { name: '5 Pack', sku: 'STICKER-GLYPH-5', priceCents: 1200, inventory: 100, attributes: { quantity: 5 } }
            ]
          }
        }
      });
    ...
    const collectionMap = Object.fromEntries(
      collections.map((c: { slug: string; id: number }) => [c.slug, c.id])
    );
    ...
    for (const product of allProducts) {
      ...
      if (product.slug.includes('hat') || product.slug.includes('snapback')) tags.push('hats');
      if (product.slug.includes('sticker')) tags.push('stickers');
      if (product.variants.some((v: { signed: boolean }) => v.signed)) tags.push('signed');
      ...
      // Link product to artist
      await prisma.productArtist.upsert({
        where: { productId_artistId: { productId: product.id, artistId: artist.id } },
        create: { productId: product.id, artistId: artist.id },
        update: {}
      });
    }
    ...
    await prisma.discount.createMany({
      data: [
        { code: 'DREAM10', type: 'percentage', value: 10, startsAt: new Date(), usageLimit: 500 },
        { code: 'FREESHIP', type: 'fixed', value: 500, startsAt: new Date(), usageLimit: 100 }
      ]
    });
    ...
    await prisma.setting.createMany({
      data: [
        {
          key: 'shipping',
          value: { domestic_standard_cents: 500, domestic_expedited_cents: 1500, international_flat_cents: 2500 }
        },
        {
          key: 'tax',
          value: { enabled: false, default_rate_percent: 8.5 }
        },
        {
          key: 'analytics',
          value: { provider: 'plausible', plausible_domain: '', ga4_measurement_id: '' }
        },
        {
          key: 'setup_checklist',
          value: {
