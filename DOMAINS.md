# Epic Dreams Domain Portfolio

This document tracks all domain assets owned by Epic Dreams Entertainment and Brand Store.

## Owned Domains

| Domain | Registrar | Status | Purpose |
|--------|-----------|--------|---------|
| epicdreamsassetmanagement.com | - | Active | Primary corporate website and merch store |
| epicdreamsaisolutions.com | Freename | Owned | AI solutions and services |

## Domain Details

### epicdreamsassetmanagement.com
- **Purpose:** Primary domain for Epic Dreams Entertainment and Brand Store
- **Deployment:** Vercel
- **DNS Configuration:** See [VERCEL_DEPLOYMENT.md](./VERCEL_DEPLOYMENT.md) for DNS setup instructions

### epicdreamsaisolutions.com
- **Registrar:** Freename
- **Purchase Date:** December 2024
- **Purpose:** AI solutions and services microsite
- **Status:** Owned, pending deployment

## Related Web Properties

- Instagram: [@epicdreamsent](https://instagram.com/epicdreamsent)
- YouTube: [@epicdreamsent](https://youtube.com/@epicdreamsent)
- TikTok: [@epicdreamsent](https://tiktok.com/@epicdreamsent)

## Notes

- Freename is a Web3 domain registrar that offers traditional and blockchain-based domain registration
- All domains should be configured with appropriate DNS records when ready for deployment
